import json, sys, os  #, openai, time
...
def LoC_level(n):
  """ LoC level descriptions from percentile bins (0-99)"""
  n = pd.to_numeric(n)
  if n <= 1:
    return "an extremely low internal locus of control"
  elif 2 <= n <= 5:
    return "a very low internal locus of control"
  elif 6 <= n <= 14:
    return "a low internal locus of control"
  elif 15 <= n <= 29:
    return "a moderately low internal locus of control"
  elif 30 <= n <= 39:
    return "a somewhat low internal locus of control"
  elif 40 <= n <= 59:
    return "a balanced (neither low nor high) internal locus of control"
  elif 60 <= n <= 69:
    return "a somewhat high internal locus of control"
  elif 70 <= n <= 84:
    return "a moderately high internal locus of control"
  elif 85 <= n <= 93:
    return "a high internal locus of control"
  elif 94 <= n <= 97:
    return "a very high internal locus of control"
  elif n >= 98:
    return "an extremely high internal locus of control"
...
def generate(temp: float, percentile: int, no_api: bool = False) -> tuple[str, Respondent]:
  ...
  with open("prompts/collect_essays_prompt.txt", "r", encoding="utf-8") as f:
    prompt = f.read().format(
      ...
      LoC_level_desc=LoC_level(percentile),
  ...
  if no_api:
    ...
    response = client.chat.completions.create(
      ...
      temperature=temp,  # default is 0.7. higher for more creative, lower for more coherent/consistent
      max_tokens=256,
...
if __name__ == "__main__":
  collect(responses_loc='essay_responses_train2')
  print('collected train')
  collect(responses_loc='essay_responses_test2')
