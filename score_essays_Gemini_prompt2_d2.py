...
CLASSIFICATION_PROMPT = """You will be given an essay written by a Chinese high school student, entitled 'My saddest experience'.
    ...
    Provide a 1 to 5 score representing the extent to which the following statement is evidenced in the student's essay.
...
EVIDENCE_PROMPT = """You retrieved this essay by a Chinese high school student that written in response to the assigned topic, 'My saddest experience': {essay}. 
    You will later be asked to choose a score, representing the extent to which the following statement is evidenced in the student's essay.
    Statement: {statement}
...
model = genai.GenerativeModel(model_name, generation_config={'temperature':0.0})  # like GPT, ranges from 0 to 2 (although default is 1)
...
df.to_csv('/Users/jw/Desktop/dt/jbs_work/Psychometrician_position/ivan proj/GEMINI_altPromptOutput_26102024.csv', index=False)
