# UPDATED SCRIPT - delete this line when u ultimately remove the hashed out outdated script
...
for i, row in all_data.iterrows():
    ...
    if (len(data_list_const_evidence) % save_every == 0):
        pd.concat([const_evidence_df, pd.DataFrame(data_list_const_evidence)]).to_csv(
            f"{output_dir}GPT_const_evidence_synth.csv", index=False)
        pd.concat([const_compare_df, pd.DataFrame(data_list_const_compare)]).to_csv(
            f"{output_dir}GPT_const_compare_synth.csv", index=False)
        pd.concat([vary_evidence_df, pd.DataFrame(data_list_vary_evidence)]).to_csv(
            f"{output_dir}GPT_vary_evidence_synth.csv", index=False)
        pd.concat([vary_compare_df, pd.DataFrame(data_list_vary_compare)]).to_csv(
            f"{output_dir}GPT_vary_compare_synth.csv", index=False)
        print(f"Autosaved after processing {i + 1} rows for this batch")
...
# # working script below - currently abandoned. to be deleted once happy with updated one.
...
#     all_data = synth_resps.merge(synth_qual_data, on='ID')
# else:  # if real data
#     all_data = pd.read_csv('"/Users/jw/Desktop/dt/jbs_work/Psychometrician_position/ivan proj/cdft_trans4mini_essay_n_sents_d2.csv"')
#     all_data.rename(columns={'trans_text': 'E'}, inplace=True)
#
# # hash out unless trialling
# all_data = all_data.head(5)  # hash out unless trialling
# # hash out unless trialling
#
...
#                                  sentence_prefix + '"' + open_qs['translation'] + '" ')
#
# # single self-written statement intended to reflect the sole trait that the test is concerned with
# consistent_statement = "the student has depression or a low mood."
# # various self-written statements intended to reflect a component of depression corresponding with the sentence
# # completion prompt or essay title
...
#         )
#         if logprob:
#             scoring_const_evidence[f"{k}T"] = int(logprob.token)
#             scoring_const_evidence[f"{k}P"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
...
#         )
#         if logprob:
#             scoring_const_compare[f"{k}T"] = int(logprob.token)
#             scoring_const_compare[f"{k}P"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#         # Varying Evidence
#         logprob = get_logprob_response(
#             CLASSIFICATION_PROMPT_EVIDENCE.format(
...
#         )
#         if logprob:
#             scoring_vary_evidence[f"{k}T"] = int(logprob.token)
#             scoring_vary_evidence[f"{k}P"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#         # Varying Comparison
#         logprob = get_logprob_response(
#             CLASSIFICATION_PROMPT_COMPARE.format(
...
#         )
#         if logprob:
#             scoring_vary_compare[f"{k}T"] = int(logprob.token)
#             scoring_vary_compare[f"{k}P"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
...
#         )
#         if logprob:
#             scoring_const_evidence[f"{k}E"] = logprob.token
#             scoring_const_evidence[f"{k}EP"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#         # Consistent Comparison
#         logprob = get_logprob_response(
...
#         )
#         if logprob:
#             scoring_const_compare[f"{k}E"] = logprob.token
#             scoring_const_compare[f"{k}EP"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#         # Varying Evidence
#         logprob = get_logprob_response(
#             SUFFINFO_PROMPT_EVIDENCE.format(
...
#         )
#         if logprob:
#             scoring_vary_evidence[f"{k}E"] = logprob.token
#             scoring_vary_evidence[f"{k}EP"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#         # Varying Comparison
#         logprob = get_logprob_response(
#             SUFFINFO_PROMPT_COMPARE.format(
...
#         )
#         if logprob:
#             scoring_vary_compare[f"{k}E"] = logprob.token
#             scoring_vary_compare[f"{k}EP"] = np.round(np.exp(logprob.logprob) * 100, 2)
#
#     # Append results
#     data_list_const_evidence.append(scoring_const_evidence)
#     data_list_const_compare.append(scoring_const_compare)
...
# const_evidence_df = pd.DataFrame(data_list_const_evidence)
# const_compare_df = pd.DataFrame(data_list_const_compare)
# vary_evidence_df = pd.DataFrame(data_list_vary_evidence)
# vary_compare_df = pd.DataFrame(data_list_vary_compare)
#
#
# # Save outputs
# output_dir = "/Users/jw/Desktop/dt/jbs_work/Psychometrician_position/ivan proj/"
...
# then, i guess it's just running it. YOU'VE PUT THAT AS TODO IN GEN_SYNTH_QUAL_RESPONSES_D2
# Yep, GEN_SYNTH_QUAL_RESPONSES_D2 is fine
...
# todo: once ok with things, then run it
